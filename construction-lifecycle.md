...
Every active operation on a frozen product type is one of five phases. There
is no sixth thing. Field declaration is passive — labeled positions in a
product type. The construction lifecycle is where computation happens.
    ...
    → Phase 2: Sealed Boundary        (model_validator mode="wrap")
      → Phase 3: Field Construction   (Pydantic parses labeled positions, from_attributes reads)
        → Phase 4: Proof Obligation   (model_validator mode="after")
          → Phase 5: Construction Effect  (model_post_init)
            → Phase 6: Derived Projection   (computed_field / cached_property, on access)
```
...
maybe one or two phases. Using all six is a signal of over-engineering.
...
## Phase 5: Construction Effect

**Pydantic hook**: `model_post_init(self, __context: object) -> None`

**What it is**: Post-construction side effect. The value exists, fields are
frozen. Construction succeeded. The effect reacts to that fact.

**The core principle**: Construction IS registration. Writing `Team(...)` declares
a team into existence. The construction effect is where that declaration takes
effect in the world — registration, indexing, notification.
...
class Resource(BaseModel, frozen=True, from_attributes=True):
    ...
    def model_post_init(self, __context: object) -> None:
        # Construction IS registration. The resource exists, so register it.
        self._registry_ctx.get().append(self)
```

**When to use**: Auto-registration on construction. Any side effect that should
happen exactly once when a value is constructed. This is the Effect building
block — construction IS activation.

---

## Phase 6: Derived Projection
...
## Completeness: Why Six Is Enough
...
| React to successful construction | Phase 5: Construction Effect (post_init) |
| Derive a view from frozen fields | Phase 6: Derived Projection (computed_field) |

There is no "run arbitrary code on a model instance" phase. Every active
computation is one of these six. Field declaration handles the passive
structure. The lifecycle handles the active computation.
...
3. **`@property` / `@computed_field`** (Phase 6) — derivation from stored fields
...
7. **`model_post_init`** (Phase 5) — side effects
